    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <div className="font-mono text-[10px] uppercase tracking-widest text-phos/80 mb-2">
          <span className="text-phos">●</span> {no} · ROLE PATH · {role}
        </div>
...
export function ThreeColumns({ happened, reveals, changes }: { happened: ReactNode; reveals: ReactNode; changes: ReactNode }) {
  return (
    <div className="mt-12 grid md:grid-cols-3 gap-6">
      {[
        { label: "WHAT HAPPENED", body: happened, color: "border-fg/30 text-fg/60" },
        { label: "WHAT IT REVEALS", body: reveals, color: "border-insight/40 text-insight" },
        { label: "WHAT CHANGES", body: changes, color: "border-phos/40 text-phos" },
      ].map((c) => (
